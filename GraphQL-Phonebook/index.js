...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      let bookArray = [];
      books.forEach((book) => {
        if (!args.genre && book.author === args.author) bookArray.push(book);

        if (!args.author && book.genres.includes(args.genre))
          bookArray.push(book);
    ...
    allAuthors: () => {
      ...
      for (let i = 0; i < authors.length; i++) {
        let author = { ...authors[i], bookAmount: 0 };
        books.forEach((book) => {
          if (book.author === author.name) author.bookAmount += 1;
        });
        returnedAuthors.push(author);
      }
      return returnedAuthors.sort((a, b) => b.bookAmount - a.bookAmount);
    },
  },
  Mutation: {
    addBook: (root, args) => {
